 ...
 *
 * The primary exit from swinging is timer-based (SWING_COOLDOWN_MS, handled
 * in the hook effect, not here). The CNN idle exit (8 frames) and timeout
 * (10 frames) remain as fallback paths.
 */
...
/** Time after entering swinging before auto-resetting to idle (ms).
 *  A full swing takes ~1.5s, plus follow-through hold. After 2s, the system
 *  resets and starts looking for the next address position. The recording
 *  post-roll handles clip completion independently. */
...
export const useSwingClassifier = ({
  ...
  useEffect(() => {
    ...
    // Timer-based exit from swinging: after SWING_COOLDOWN_MS, reset to idle
    // and start looking for the next address. The recording post-roll handles
    // clip completion independently — this just cycles the state machine.
    if (newState.detectionState === 'swinging' && newState.swingStartTimestamp) {
      const swingDuration = now - newState.swingStartTimestamp;

      if (swingDuration >= SWING_COOLDOWN_MS) {
        if (__DEV__) {
          console.log(
            `[SwingClassifier] COOLDOWN EXIT: swinging → idle after ${swingDuration}ms`,
          );
        }
        event = {
          type: 'swingEnded',
