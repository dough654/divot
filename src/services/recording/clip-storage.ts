...
export const updateClip = async (
  clipId: string,
  fields: Partial<Pick<Clip, 'tempoRatio' | 'backswingDurationMs' | 'downswingDurationMs' | 'takeawayTimestampMs' | 'peakTimestampMs' | 'impactTimestampMs' | 'name' | 'cameraAngle' | 'cloudClipId' | 'syncStatus'>>,
): Promise<Clip | null> => {
  const metadata = await loadMetadata();
